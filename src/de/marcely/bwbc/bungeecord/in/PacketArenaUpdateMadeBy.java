...
extends Packet {
    ...
    public String getPacketString() {
        return PacketType.IN_PacketArenaUpdateMadeBy.getID() + "/" + SLASH.matcher(arena.getName()).replaceAll(Matcher.quoteReplacement("&sKEYslash;")) + "/" + SLASH.matcher(to).replaceAll(Matcher.quoteReplacement("&sKEYslash;"));
    }
    ...
    public static PacketArenaUpdateMadeBy build(final String str) {
        ...
        if (strs.length == 3) {
            ...
            if (arena != null) {
                return new PacketArenaUpdateMadeBy(arena, KEYSLASH.matcher(strs[2]).replaceAll(Matcher.quoteReplacement("/")));
            }
            return null;
        }
        return null;
