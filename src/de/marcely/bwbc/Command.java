...
import de.marcely.bwbc.nms.NMSHandler;
...
implements CommandExecutor {
    ...
    private boolean cmd(final CommandSender sender, final String label, final String[] args) {
        if (args.length >= 1) {
            ...
            if (cmd.equalsIgnoreCase("summon") && Util.hasPermission(sender, Permission.Command_Summon)) {
                if (sender instanceof Player) {
                    final Player player = (Player)sender;
                    if (args.length >= 4 && args[1].equalsIgnoreCase("lobbyvillager")) {
                        if (args[2].equalsIgnoreCase("specify")) {
                            final Arena arena = Util.getArena(args[3]);
                            if (arena != null) {
                                spawningCreature.add(player);
                                final Entity e = player.getWorld().spawnEntity(player.getLocation(), EntityType.VILLAGER);
                                e.setCustomName(Util.config_lobbyVillagerPrefix + arena.getName());
                                NMSHandler.setNoAI(e);
                                player.sendMessage(Language.Spawned_LobbyVillager.getMessage());
                            } else {
                                player.sendMessage(ARENA.matcher(Language.NotFound_Arena.getMessage()).replaceAll(Matcher.quoteReplacement(args[3])));
                            }
                        } else if (Util.isInteger(args[2])) {
                            if (Util.isInteger(args[3])) {
                                spawningCreature.add(player);
                                final Entity e = player.getWorld().spawnEntity(player.getLocation(), EntityType.VILLAGER);
                                e.setCustomName(Util.config_lobbyVillagerPrefix + args[2] + "x" + args[3]);
                                NMSHandler.setNoAI(e);
                                player.sendMessage(Language.Spawned_LobbyVillager.getMessage());
                            } else {
                                player.sendMessage(NUMBER.matcher(Language.Number_NotOne.getMessage()).replaceAll(Matcher.quoteReplacement(args[3])));
                            }
                        } else {
                            player.sendMessage(NUMBER.matcher(Language.Number_NotOne.getMessage()).replaceAll(Matcher.quoteReplacement(args[2])));
                        }
                    } else {
                        sender.sendMessage(Language.All_Commands.getMessage());
                        sender.sendMessage(ChatColor.AQUA + "/" + label + " summon lobbyvillager <teams> <playersInTeam>");
                        sender.sendMessage(ChatColor.AQUA + "/" + label + " summon lobbyvillager specify <arena name>");
                    }
                } else {
